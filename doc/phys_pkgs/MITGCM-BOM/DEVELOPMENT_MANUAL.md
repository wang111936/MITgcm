...
### Phase 5：科学验收与 HPC 加固

交付：

- 标准 `mitgcmuv`、真实输入文件和非零时长的生产积分闭环；
- `JULIA` 模式完整轨迹与锁定 Julia fixed-step golden 对照；
- `PAPER2024` 独立方程 oracle 和 RK4 时间步收敛；
- spring、生物、陆地、事件、schema-4、MPI/restart 的实际多步算例；
- 动态海洋 one-way 不扰动验证和 30 天小型耐久运行；
- 通信与负载分析；
- 轨迹分片和后处理；
- OpenMP 安全；
- $10^5$ 粒子扩展测试；
...
科学准入条件：P5-SA-G99 与独立审计通过，且不得用 `JULIA` oracle
判定 `PAPER2024`。Phase 5 最终退出条件：B20 达标，连续/重启长实验
一致，内存和通信无非线性增长；本地科学准入不能替代目标服务器验收。
