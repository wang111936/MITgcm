...
| P1.1 状态与初值 | 待复审 | `MITGCM-BOM/phase-01-state` / PR #8 | 4 项审查阻断已关闭；扩展门禁与 Phase 0 回归通过 |
...
- 读取受 `bomInitGlobalLimit` 约束的 schema 1 MDS 全局初值文件，并交叉验证 meta 维度、float64 精度、记录数和 `BOMV0001` schema；
...
- 权威 P1.1 测试 ID：`p11-state-review-fixes-attempt03`；
...
- 19/19 负向门禁通过，新增 missing meta、bad meta schema 和 trailing record；
...
- 最终 Phase 0 回归 ID：`p11-review-fixes-phase0-attempt01`，锁定参考、离线 Julia、8/8 smoke、P0.4 的 4 构建/3 正向/2 负向全部通过；
...
- 所有目录均保留且未覆盖，只有 `p11-state-review-fixes-attempt03` 是当前权威证据。
...
- 生产/门禁 head：`MITGCM-BOM/phase-01-state@2c688a7e90d1bdd814a8bd8b0ef5db63c7d67a65`；
...
1. 推送修复和记录提交后，复核 PR #8 相对 `MITGCM-BOM/development` 的 31 文件独立差异和设计边界；
2. 独立复审四项阻断的修复、评审线程、提交状态与 base/head SHA，不把 draft 自动标记 ready；
