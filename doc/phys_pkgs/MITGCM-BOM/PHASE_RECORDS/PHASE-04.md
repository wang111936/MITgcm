...
| 工作包 | 内容 | 状态 | 退出门禁 |
|---|---|---|---|
| P4.0 | 源码审计、接口/编号/schema/RNG/测试冻结 | 已集成 | 文档/范围审计 15/15；生产 diff 为零 |
| P4.1 | 参数/稳定码、T/N 端点、Brooks stateless kernel | 已集成 | P4-Z01/C01/E01/E02/B01、B12、前序 538 |
...
6. `p44-final-head-attempt05` 在洁净精确头通过 57/57，覆盖 NONE/EBOMB
   x serial/MPI2/MPI4 continuous/split bitwise restart、16 类损坏和两类
   changed-decomposition 拒绝；
7. `p44-p43-accepted-attempt05`、`p44-p42-accepted-attempt05`、
   `p44-p41-accepted-attempt05` 分别通过 26/26、18/18、31/31；P4.1
   使用显式 `p44` scope 区分新增合法容量与仍须 pre-init 拒绝的缺失初态；
8. 隔离 shared clone 的 `p44-predecessor-final-head-attempt05` 通过
   Phase 3 direct 148/148 与 Phase 2 closure 390/390，总计 538/538；
9. P4.2 回放发现两个直接构造状态的隔离夹具未同步 P4.4 next-ID/event
   账本不变量；已修正夹具边界，未放宽生产预算检查；
10. predecessor 性能审计与最终 P3-G99 独立审计均冻结 P4 高/低字
    exact-count gather 的 shape/count；三份容量测试头同步 P4.4 schema
    常量且仅保留原容量差异；
11. BOM 仍为独立 package，未读写 SKRIPS；未创建 v0.5。
