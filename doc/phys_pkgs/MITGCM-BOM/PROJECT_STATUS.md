...
| 项目 | 当前值 |
|---|---|
| 最后更新 | 2026-09-15（社区入口与双语首页；不改变模型源码和测试基线） |
| 权威开发仓库 | `/home/wyl/projects/mitgcm-bom` |
| GitHub 仓库 | `wang111936/MITgcm` |
| 上游仓库 | `MITgcm/MITgcm` |
| 集成分支 | `MITGCM-BOM/development` |
| 当前任务分支 | 社区入口来源 `MITGCM-BOM/community-entry-20260915`；交付目标 `MITGCM-BOM/development` |
| 当前开发源码提交 | 预发布 merge `53b01e221cf48e03b9cf944f769fc225b4d5f8e8`；生产代码/verification 与四组专项提交 `ddc8699d7ea1996efc91d9b531b9bb7eeeba5c73` 一致；本轮仅文档 |
| GitHub development | PR #46 已合并为 `53b01e221`；本轮社区入口 merge 的 SHA/PR 以集成分支历史为准，不替换科学测试绑定版本 |
| 独立实验源码 | `/home/wyl/MITGCM-BOM/src`：detached `00ce0c391` + ARCHIVE + 未提交 CAL–EXF 修复；不是主仓库的干净镜像 |
| 当前阶段 PR | PR #46 预发布已完成；社区入口采用单 PR/merge commit，不创建标签 |
| 当前阶段 | Phase 5：科学验收与 HPC 加固（科学准入完成；HPC 未评估） |
| 当前工作包 | 2026-09-15 双语社区首页、作者与联系信息、可发现性配置；模型基线仍为 2026-09-14 预发布，不是完整 Phase 5/HPC/v1.0 退出 |
...
1. PR #46 已以 merge commit 发布为 `53b01e221`，包含 P5.5、ARCHIVE、
   CAL–EXF 和文档/证据。2026-09-15 仅完善双语首页和社区发现入口，
   明确研究问题、与 FLT 的差异、王煜林博士的单位署名和联系邮箱；不新增物理功能。
...
3. 同一干净候选提交已通过 endpoint 40/40、pickup 10/10、ARCHIVE 基础
   10/10 与 active 6/6；预发布 merge 已核验代码树一致性。
...
### 2026-09-15：社区入口与双语首页

- 英文 `README.md` 与中文 `README.zh-CN.md` 先说明表面漂浮物输运问题，
  再比较 BOM 与 FLT 的用途、动力学、可选过程和独立性，并提供可运行教程。
- 明确署名：由广东海洋大学海洋与气象学院王煜林博士开发；联系邮箱
  `wang111936@outlook.com`。该署名指 BOM 新增实现和集成，不替代上游模型、
  理论与参考软件作者；保留原许可证及来源链接。
- 移除新首页中的上游构建、文档、DOI 徽章和工具宣传署名；不删除历史记录，
  不将上游认证或引用标识冒用为 BOM 本身的认证或成果。
- 社区发布配置包含 BOM 专属简介、使用指南入口、18 项研究/技术 Topics、
  问题反馈入口和默认展示 `MITGCM-BOM/development`；原 `master` 保留。
  配置属于仓库元数据，最终生效情况需以发布核验为准，搜索排名不作保证。
- 核验范围是双语内容、相对链接、署名/邮箱、无徽章、文档差异和生产代码树
  一致性；不重跑模型积分，也不新增科学验收声明。所有未实现/延期条目保留。
...
- `p44-final-head-attempt05` 在洁净精确头通过 57/57；
- `p44-p43-accepted-attempt05`、`p44-p42-accepted-attempt05`、
  `p44-p41-accepted-attempt05` 分别通过 26/26、18/18、31/31；
- P4.1 使用显式 `p44` scope，保留原配置失败矩阵，并把新增合法正容量与
  仍须在初始化前拒绝的缺失初始状态分开审计；
- 隔离 shared clone 的 `p44-predecessor-final-head-attempt05` 通过
...
- exact RNG 提交 `852659089`、birth/order 提交 `7a14707da`、
  atomic event/schema3/graph 提交 `460d42890`；
...
- 根据用户要求，新增中英双语研究入口、BOM/FLT 区别、开发者单位姓名和邮箱；
  增加准确研究关键词，去除首页徽章与工具宣传，不清空历史和上游版权。
- 采用 `MITGCM-BOM/community-entry-20260915` 集中交付，作者为
  `WangYuLin <wang111936@outlook.com>`；仅更新首页和本账本。
- 本轮不开展源码实现或模型积分；D08、S1、事件 I/O、HPC、MNC 等状态不变。
  默认展示分支与发现/反馈设置随文档发布核验；不新建标签或正式 Release。
...
### 2026-08-29：P4.2 集成与 P4.3 本地完成

- P4.2 PR #35 已以 merge commit `f7937de95f7637b9b30ff8f08fb5e20d77f4f360`
  集成，从该头创建 `MITGCM-BOM/p4.3-rng-birth-schema3`；
- `852659089` 实现 exact Philox4x32-10 与 frozen birth key，
  `7a14707da` 实现 deterministic retry/global parent order/child IDs，
  `460d42890` 实现 atomic birth/death/free-stack、packet schema 3 和
  post-event graph/component rebuild；
- 对照冻结计划后，将候选门禁从 23 行补全为 26 行：增加
  MPI2/MPI4 exact Philox、schema/word/count/destination/real/length packet
  mutations 以及 reverse owner-slot owner/event 规范化比较；
- P4.3 production migration 共用新的 packet validator，所有验证发生在
  owner replacement 之前；有效 schema-2 10/6 和 schema-3 13/7 路径
  都通过；
...
### 2026-08-29—30：P4.3 集成与 P4.4 本地完成
...
- clean closeout 证据为 `p44-final-head-attempt05` 57/57、
  `p44-p43-accepted-attempt05` 26/26、`p44-p42-accepted-attempt05`
  18/18、`p44-p41-accepted-attempt05` 31/31 和
  `p44-predecessor-final-head-attempt05` 538/538；
...
- P3.5 predecessor 与最终 P3-G99 独立审计均冻结 P4 高/低字 exact-count
  gather 的 shape/count；三份容量覆盖头同步 schema 常量，只保留容量差异；
