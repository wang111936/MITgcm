...
3. 核对 `p44-final-head-attempt04` 的 57/57、
   `p44-p43-accepted-attempt04` 的 26/26、
   `p44-p42-accepted-attempt04` 的 18/18、
   `p44-p41-accepted-attempt04` 的 31/31 与
   `p44-predecessor-final-head-attempt04` 的 538/538、source-head 和
   manifest 自校验；
...
- `p44-final-head-attempt04` 在洁净精确头通过 57/57；
- `p44-p43-accepted-attempt04`、`p44-p42-accepted-attempt04`、
  `p44-p41-accepted-attempt04` 分别通过 26/26、18/18、31/31；
- P4.1 使用显式 `p44` scope，保留原配置失败矩阵，并把新增合法正容量与
  仍须在初始化前拒绝的缺失初始状态分开审计；
- 隔离 shared clone 的 `p44-predecessor-final-head-attempt04` 通过
...
- clean closeout 证据为 `p44-final-head-attempt04` 57/57、
  `p44-p43-accepted-attempt04` 26/26、`p44-p42-accepted-attempt04`
  18/18、`p44-p41-accepted-attempt04` 31/31 和
  `p44-predecessor-final-head-attempt04` 538/538；
...
- P3.5 predecessor 性能审计冻结 P4 高/低字 exact-count gather 的 shape/
  count；三份容量覆盖头同步 P4.4 schema 常量，只保留既有容量差异；
