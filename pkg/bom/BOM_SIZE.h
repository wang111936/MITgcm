CBOP
C     !ROUTINE: BOM_SIZE.h
C     !INTERFACE:
C     #include "BOM_SIZE.h"

C     !DESCRIPTION:
C     Compile-time storage limits for pkg/bom.  P1.4 adds bounded exact-ID
C     owner exchange packets without changing the compact tile capacity.
CEOP

      INTEGER bomMaxPartTile
      INTEGER bomMaxGhostTile
      INTEGER bomMaxExchange
      INTEGER bomMaxNeighbor
      INTEGER bomMaxEventBuffer
      INTEGER bomMaxInitRecords
      INTEGER bomPacketSchema
      INTEGER bomPacketInts
      INTEGER bomPacketReals
      INTEGER bomGhostSchema
      INTEGER bomGhostPacketInts
      INTEGER bomGhostPacketReals
      INTEGER bomGhostPacketBytes
      INTEGER bomComponentLabelSchema
      INTEGER bomComponentLabelPacketInts
      INTEGER bomComponentLabelPacketBytes
      INTEGER bomComponentSizeSchema
      INTEGER bomComponentSizePacketInts
      INTEGER bomComponentSizePacketBytes
      INTEGER bomInitialSchema
      INTEGER bomInitialFields
      INTEGER bomOutputSchema
      INTEGER bomOutputFields
      INTEGER bomOutputSchema2
      INTEGER bomOutputFields2
      INTEGER bomP3ContainerSchema
      INTEGER bomP3SidecarSchema
      INTEGER bomP3SidecarFields
      INTEGER bomP3SigBase
      INTEGER bomPickupSchema
      INTEGER bomPickupFields
      INTEGER bomPickupFields2
      INTEGER bomPickupSigFields
      INTEGER bomPickupSchema2
      INTEGER bomPickup2SigBase
      INTEGER bomPickupEnvHeader
      INTEGER bomPickupChunkFields
      INTEGER bomP3CounterFields
      INTEGER bomP3CounterDigits
      INTEGER BOM_P3_COUNT_OWNER
      INTEGER BOM_P3_COUNT_GHOST
      INTEGER BOM_P3_COUNT_NONEMPTY
      INTEGER BOM_P3_COUNT_COMPARISON
      INTEGER BOM_P3_COUNT_DIRECTED
      INTEGER BOM_P3_COUNT_UNDIRECTED
      INTEGER BOM_P3_COUNT_MAX_NEIGHBOR
      INTEGER BOM_P3_COUNT_REBUILD
      INTEGER BOM_P3_COUNT_PACKET_SENT
      INTEGER BOM_P3_COUNT_PACKET_RECEIVED
      INTEGER BOM_P3_COUNT_BYTE_SENT
      INTEGER BOM_P3_COUNT_BYTE_RECEIVED
      INTEGER BOM_P3_COUNT_MAX_COMPONENT
      INTEGER BOM_P3_COUNT_COMPONENT_ITER
      PARAMETER ( bomMaxInitRecords = 10000 )
      PARAMETER ( bomMaxPartTile    = 64 )
      PARAMETER ( bomMaxGhostTile   = bomMaxInitRecords )
      PARAMETER ( bomMaxExchange    = bomMaxInitRecords )
      PARAMETER ( bomMaxNeighbor    = bomMaxInitRecords )
      PARAMETER ( bomMaxEventBuffer = bomMaxInitRecords )
      PARAMETER ( bomPacketSchema   = 2 )
      PARAMETER ( bomPacketInts     = 10 )
      PARAMETER ( bomPacketReals    = 6 )
      PARAMETER ( bomGhostSchema    = 1 )
      PARAMETER ( bomGhostPacketInts = 9 )
      PARAMETER ( bomGhostPacketReals = 2 )
      PARAMETER ( bomGhostPacketBytes =
     &            4*bomGhostPacketInts+8*bomGhostPacketReals )
      PARAMETER ( bomComponentLabelSchema = 1 )
      PARAMETER ( bomComponentLabelPacketInts = 10 )
      PARAMETER ( bomComponentLabelPacketBytes =
     &            4*bomComponentLabelPacketInts )
      PARAMETER ( bomComponentSizeSchema = 1 )
      PARAMETER ( bomComponentSizePacketInts = 5 )
      PARAMETER ( bomComponentSizePacketBytes =
     &            4*bomComponentSizePacketInts )
      PARAMETER ( bomInitialSchema  = 1 )
      PARAMETER ( bomInitialFields  = 8 )
      PARAMETER ( bomOutputSchema   = 1 )
      PARAMETER ( bomOutputFields   = 24 )
      PARAMETER ( bomOutputSchema2  = 2 )
      PARAMETER ( bomOutputFields2  = 48 )
      PARAMETER ( bomP3ContainerSchema = 3 )
      PARAMETER ( bomP3SidecarSchema = 1 )
      PARAMETER ( bomP3SidecarFields = 8 )
      PARAMETER ( bomP3SigBase = 44 )
      PARAMETER ( bomPickupSchema   = 1 )
      PARAMETER ( bomPickupFields   = 24 )
      PARAMETER ( bomPickupFields2  = 45 )
      PARAMETER ( bomPickupSigFields = 16 )
      PARAMETER ( bomPickupSchema2  = 2 )
      PARAMETER ( bomPickup2SigBase = 53 )
      PARAMETER ( bomPickupEnvHeader = 12 )
      PARAMETER ( bomPickupChunkFields = 24 )
      PARAMETER ( bomP3CounterFields = 14 )
      PARAMETER ( bomP3CounterDigits = 5 )
      PARAMETER ( BOM_P3_COUNT_OWNER = 1 )
      PARAMETER ( BOM_P3_COUNT_GHOST = 2 )
      PARAMETER ( BOM_P3_COUNT_NONEMPTY = 3 )
      PARAMETER ( BOM_P3_COUNT_COMPARISON = 4 )
      PARAMETER ( BOM_P3_COUNT_DIRECTED = 5 )
      PARAMETER ( BOM_P3_COUNT_UNDIRECTED = 6 )
      PARAMETER ( BOM_P3_COUNT_MAX_NEIGHBOR = 7 )
      PARAMETER ( BOM_P3_COUNT_REBUILD = 8 )
      PARAMETER ( BOM_P3_COUNT_PACKET_SENT = 9 )
      PARAMETER ( BOM_P3_COUNT_PACKET_RECEIVED = 10 )
      PARAMETER ( BOM_P3_COUNT_BYTE_SENT = 11 )
      PARAMETER ( BOM_P3_COUNT_BYTE_RECEIVED = 12 )
      PARAMETER ( BOM_P3_COUNT_MAX_COMPONENT = 13 )
      PARAMETER ( BOM_P3_COUNT_COMPONENT_ITER = 14 )

C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
