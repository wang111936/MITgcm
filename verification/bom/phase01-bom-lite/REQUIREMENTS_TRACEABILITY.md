...
状态：P1.1 已验收；P1.2 P1-R05—P1-R07 实现、全回归、独立复审及 PR #10—#12 收口全部完成；P1.3 单 tile 接口已冻结，setup、无状态 Leeway RHS、无状态 RK2 与 P1-I05 三个生产增量均已通过精确提交专属门禁及全部前序回归；RK4、release 状态机、事务提交与完整状态预算仍待后续增量
...
| P1-R08 | LEEW 海流 RHS 使用 SI 且坐标率转换正确 | `BOM_RHS_LEEWAY` | P1-I01、P1-I02、P1-I03 | P1.3 | RHS 组件、Cartesian 常值 RK2 解析位移和 P1-I05 二阶收敛完成；spherical/生产生命周期轨迹待后续 |
...
| P1-R11 | 固定子步、显式中点 RK2 和经典 RK4 | `BOM_MAIN`、`BOM_RK2`、`BOM_RK4` | P1-S04b、P1-I05、P1-I06 | P1.3 | 子步安全导出与无状态 RK2 组件完成；P1-I05 最细两阶为 1.9885/1.9942；RK4、release 和生产子步调用待后续 |
...
| P1-R16 | 运行中检查有限数、CFL、owner 数和状态预算 | `BOM_RHS_LEEWAY`、`BOM_CHECK_STATE`、`BOM_MAIN` | P1-N08、P1-X03、P1-G01 | P1.3–P1.5 | 启动 preflight、RHS 稳定失败类别及 RK2 NONE/K1/K2/FINAL 首失败/回滚门禁完成；生产调用层集体诊断和全局 owner/ID/状态预算待后续 |
