...
状态：P1.1 已验收；P1.2 P1-R05—P1-R07 实现、全回归、独立复审及 PR #10—#12 收口全部完成；P1.3 单 tile 接口已冻结，setup 与无状态 Leeway RHS 已通过专属门禁和全部前序回归，无状态 RK2 及 P1-I05 门禁已实现、精确提交证据待执行；RK4、release 状态机、事务提交与完整状态预算仍待后续增量
...
| P1-R08 | LEEW 海流 RHS 使用 SI 且坐标率转换正确 | `BOM_RHS_LEEWAY` | P1-I01、P1-I02、P1-I03 | P1.3 | RHS 组件完成；Cartesian m/s、spherical degree/s、零场和串行/MPI4 门禁通过；位置解析轨迹待 RK 增量确认 |
| P1-R09 | 可选 EXF 风按独立经验系数叠加 | `BOM_CHECK`、`BOM_BUILD_FIELDS`、`bomGridWind*`、`BOM_RHS_LEEWAY` | P1-I04、P1-N06 | P1.3 | source/依赖矩阵、冻结风场与 EXF→field→RHS 串行/MPI4 端到端 P1-I04 完成；轨迹层验收待 RK 增量 |
| P1-R10 | Stokes 在 Phase 1 固定关闭，误配置失败 | `BOM_READPARMS`、`BOM_CHECK` | P1-N07 | P1.1 | P1.1 完成 |
| P1-R11 | 固定子步、显式中点 RK2 和经典 RK4 | `BOM_MAIN`、`BOM_RK2`、`BOM_RK4` | P1-S04b、P1-I05、P1-I06 | P1.3 | 子步安全导出已完成启动 preflight；无状态 RK2 与 P1-I05 门禁已实现、精确提交证据待执行；RK4 与生产子步调用待后续 |
...
| P1-R16 | 运行中检查有限数、CFL、owner 数和状态预算 | `BOM_RHS_LEEWAY`、`BOM_CHECK_STATE`、`BOM_MAIN` | P1-N08、P1-X03、P1-G01 | P1.3–P1.5 | 启动 preflight、RHS 稳定失败类别及 RK2 首失败 stage/回滚已实现；精确 RK2 证据、调用层集体诊断和全局 owner/ID/状态预算待后续 |
...
| `BOM_RK_COORD_UPDATE`、`BOM_RK2` | P1-R11、P1-R16 | 第三增量已实现无状态显式中点、K1/K2/FINAL 全 RHS、溢出安全试算和失败回滚；P1-I05/阶段失败精确提交证据待执行 |
